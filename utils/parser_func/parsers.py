...
from utils.common.helper import message_to_site

# from config.config import orderbook, lock
from config.config import state_arbitrage
from utils.common.candles import update_candle
    ...
    for symbol, exchanges in пары.items():
        min_exchange, min_market_type, min_price, funding, timer, all_data = min(
            (
                (
                    ...
                    all_data.get("funding", 0),
                    all_data.get("get_funding", "spot"),
                    all_data,
                )
        ...
        for exchange, types in exchanges.items():
            for market_type, data in types.items():
                ...
                if min_exchange == exchange or (min_market_type, market_type) in [
                    ("spot", "spot"),
                    ("futures", "spot"),
                ]:
                    continue
                ...
                if (spread >= 1 or spred_funding + spread >= 1) and (
                    exchange == "mexc"
                    or (data.get("funding") and data.get("get_funding"))
                ):
                    if symbol not in subscriptions_config:
                        subscriptions_config[symbol] = {}
                    if exchange not in subscriptions_config[symbol]:
                    ...
                    if (
                        min_market_type
                        not in subscriptions_config[symbol][min_exchange]
                    ):
                        subscriptions_config[symbol][min_exchange][
                            min_market_type
                        ] = all_data
        ...
        while True:
                            ...
                            for i, pos_sell in enumerate(all_positions):
                                for j, pos_buy in enumerate(all_positions):
                                    if i == j or (
                                        (
                                            pos_buy["market_type"],
                                            pos_sell["market_type"],
                                        )
                                        in [("spot", "spot"), ("futures", "spot")]
                                    ):
                                        continue

                                    exit_long = (
                                        (pos_buy["sell_avg"] - pos_buy["buy_avg"])
                                        / pos_buy["buy_avg"]
                                    ) * 100
                                    exit_short = (
                                        (pos_sell["sell_avg"] - pos_sell["buy_avg"])
                                        / pos_sell["sell_avg"]
                                    ) * 100
                                    ...
                                    комиссии = (
                                        pos_sell["fee_taker"] + pos_buy["fee_taker"]
                                    )
                                    funding_spread = (
                                        pos_sell["funding"] - pos_buy["funding"]
                                    )

                                    курсовой = (
                                        (pos_sell["sell_avg"] - pos_buy["buy_avg"])
                                        / pos_buy["buy_avg"]
                                    ) * 100

                                    spread_total = (
                                        курсовой - комиссии + funding_spread + total
                                    )
                                    ...
                                    if (
                                        spread_total >= spread
                                        and pos_sell["exchange"] in list_exchanges
                                        and pos_buy["exchange"] in list_exchanges
                                        ...
                                        if admin == True:
                                            time_candle = int(time.time())
                                            await update_candle(
                                                key=key,
                                                time=time_candle,
                                                spread=spread_total,
                                                exchange_long=pos_buy["exchange"],
                                                type_long=pos_buy["market_type"],
                                                exchange_short=pos_sell["exchange"],
                                                type_short=pos_sell["market_type"],
                                                volume=volume, 
                                                symbol=symbol
                                            )
                                        ...
                                        все_возможности[symbol].append(
                                            {
                                                ...
                                                "funding_spread": round(
                                                    funding_spread, 2
                                                ),  #
                                                "курсовой": round(курсовой, 2),  #
                                                "курсовой_с_тоталом": round(
                                                    spred_without_fund, 2
                                                ),  #
                                                "funding_long": pos_buy["funding"],
                                                "funding_long_time": pos_buy[
                                                    "funding_time"
                                                ],
                                                "funding_short": pos_sell["funding"],
                                                "funding_short_time": pos_sell[
                                                    "funding_time"
                                                ],
                                                "volume": round(
                                                    (volume / pos_buy["buy_avg"]), 4
                                                ),  #
                                                'volume_usdt': volume,
            ...
            try:
                for symbol_for_send, data_for_send in все_возможности.items():
                    await message_to_site(
                        symbol=symbol_for_send, user_id=user_id, data=data_for_send
                    )
                    # print(f'\n\n\n{symbol_for_send}\n{data_for_send}\n\n\n')
                for symbol_for_delete in delete:
                    await message_to_site(symbol=symbol_for_delete, user_id=user_id)
            except Exception as e:
                logger.error(f"Ошибка в отправление/удаления сообщения в функции арбитража {e}", exc_info=True)

    except Exception as e:
        logger.error(f"Ошибка в функции арбитража {e}", exc_info=True)