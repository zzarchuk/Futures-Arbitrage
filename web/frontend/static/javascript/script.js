...
ws.onmessage = (event) => {
    ...
    tbody.insertAdjacentHTML("beforeend", `
        <tr>
            <td colspan="9"><strong>${symbol}</strong></td>
        </tr>
        <tr>
            <td>Лонг / Шорт</td>
            <td>Цены</td>
    ...
    data.forEach(item => {
        tbody.insertAdjacentHTML("beforeend", `
            <tr>
                ...
                <td>${item.volume}</td>
                <td>${item.lifetime ?? ""}</td>
                <td><button onclick="openChart('${symbol}', '${item.exchange_long}', '${item.long_type}', '${item.exchange_short}', '${item.short_type}')">📈</button></td>
                <td><button onclick="openPosition('${symbol}', '${item.exchange_long}', '${item.long_type}', '${item.exchange_short}', '${item.short_type}', '${item.volume}')">💹</button></td>
            </tr>
        `);
    });
...
async function openChart(symbol, long, type_long, short, type_short) {
    window.open(`/chart?symbol=${symbol}`, '_blank');
}
