...
черный_список = ["ALLUSDT", "AVNTUSDT", 'SVSAUSDT', 'NUMIUSDT', 'MEGAUSDT']
...
async def safe_fetch_order_book(
    ...
    # sem = semaphores.get(exchange, asyncio.Semaphore(20))
    # sem = semaphores.get(exchange)
    ...
    while True:
        async with semaphore:
            ...
            if возможности:  # Если спред найден
                ...
                # Если сообщение никогда не отправлялось - просто выходим
                if айдишники is None:
                    return
                
                # Если 5 итераций подряд нет спреда
                if пустых_итераций >= 5:
                    try:
                        # Пытаемся отредактировать с пометкой о потере
                        if msg_for_edit is not None:
                            ...
                            # Если текста нет (не должно быть, но на всякий случай) - удаляем
        ...
        if текущий_словарь:
            ...
            for пара in chunk_dict(текущий_словарь):
                task = asyncio.create_task(
                    арбитраж(пара, биржи, 100, 50, 300, 25, session)
                )
                tasks.append(task)
...
async def on_startup():
    ...
    # Запускаем фоновые задачи и передаём sessions
